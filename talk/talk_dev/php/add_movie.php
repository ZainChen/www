...
*/
  ...
  $nickname = $_POST['txt_title'];  //获取公告标题信息
  $content = $_POST['txt_content'];  //获取公告内容
  $createtime = date("Y-m-d H:i:s");  //获取系统当前时间,H代表24小时制,需大写
  /*********应用mysql_query()函数执行insert...into语句发送到服务器***********/
  $sql = mysqli_query($conn, "insert into tb_talk(nickname,content,createtime)values('$nickname','$content','$createtime')") or die("数据表访问错误,请检查该表是否存在".mysqli_error());
